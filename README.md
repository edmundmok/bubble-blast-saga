...
3. **Added a simple hint system to inform user of possible locations to shoot at.** This system only hints at possible locations to shoot at, not actually checking which location can maximize the result. I tried to do the latter, but it was too computationally expensive as we need to simulate the actual interactions in order to determine the outcome for each candidate position. The result is that it will be quite laggy. For this simple hint system, I only needed to identify all the exposed bubbles (bubbles that can be reached somehow) and check if the neighbour is a special bubble or same color as the current cannon bubble. If any of these are true, the position is possible hint position.
